...
const StockInfo = ({ stock_details, show }) => {
    ...
    return (
        <View>
            <View style={{ flexDirection: "row", justifyContent: 'space-between', backgroundColor: '#fff', padding: 12 }} >
                <View>
                    {show && <><Text style={{ fontSize: 20, fontWeight: '700', color: 'black', marginTop: 10, }} >CurrentValue:</Text>
                ...
                <View>
                    {show && <><Text style={{ fontSize: 20, color: 'black', fontWeight: '500', marginTop: 10, alignSelf: 'flex-end' }} >₹{stockCalculator('current_sum')?.toFixed(2)}</Text>
